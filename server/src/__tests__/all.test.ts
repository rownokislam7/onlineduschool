...
const examFindUniqueMock = jest.fn().mockResolvedValue(null);
...
jest.mock("@prisma/client", () => ({
  PrismaClient: jest.fn().mockImplementation(() => ({
    ...
    exam: { findUnique: examFindUniqueMock, findMany: jest.fn().mockResolvedValue([]), count: jest.fn().mockResolvedValue(0) },
...
describe("bulkImportQuestions", () => {
  ...
  it("imports tilde-delimited CSV rows with commas in question text", async () => {
    const { bulkImportQuestions } = await import("../services/adminService");
    const examFindUnique = jest.requireMock("@prisma/client").PrismaClient().exam.findUnique;
    examFindUnique.mockResolvedValue({ id: "e-1", slug: "test-exam", questions: [] });

    const csv = "question~type~options~correct~marks\nWhat is the purpose of the C function printf, and how is it used?~SHORT_ANSWER~~printf is used to print formatted output~2";
    const result = await bulkImportQuestions("test-exam", csv);
    expect(result).toEqual({ imported: 1 });
  });

  it("imports tilde-delimited CSV rows with embedded quote characters", async () => {
    const { bulkImportQuestions } = await import("../services/adminService");
    const examFindUnique = jest.requireMock("@prisma/client").PrismaClient().exam.findUnique;
    examFindUnique.mockResolvedValue({ id: "e-1", slug: "test-exam", questions: [] });

    const csv = "question~type~options~correct~marks\n\"\"\"রোবট\"\" শব্দটি প্রথম ব্যবহার করেন কে?~MCQ~আইজ্যাক আসিমভ|কারেল চাপেক|অ্যালান টুরিং|জন ম্যাকার্থি~1~1";
    const result = await bulkImportQuestions("test-exam", csv);
    expect(result).toEqual({ imported: 1 });
  });
});
