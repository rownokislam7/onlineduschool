...
export async function bulkImportQuestions(examSlug: string, csvContent: string) {
  ...
  const expectedColumns = ["question", "type", "options", "correct", "marks"];
  const missingColumns = expectedColumns.filter((col) => !headerKeys.includes(col));
  if (missingColumns.length) {
    throw new AppError(
      `Invalid CSV header. Use "~" to separate columns: question~type~options~correct~marks`,
      422
    );
  }
